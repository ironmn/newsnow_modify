...
export const BASE_URL = "https://news.cctv.com"
...
export function createCctvSource({
  ...
  return defineSource(async () => {
    ...
    if (newsMap.size === 0) {
      for (const endpoint of htmlPaths) {
        try {
    ...
    if (sorted.length === 0) throw new Error(errorMessage)

    return sorted.slice(0, 30)
  })
}

export type CreateCctvSourceOptions = Parameters<typeof createCctvSource>[0]
